...
object QueryShortChannelIdsTlv {
  ...
  case object QueryFlagType {
    ...
    def includeChannelAnnouncement(flag: Long) =
    ...
    def includeNodeAnnouncement1(flag: Long) =
      (flag & INCLUDE_NODE_ANNOUNCEMENT_1) != 0

    def includeNodeAnnouncement2(flag: Long) =
  ...
  val encodedQueryFlagsCodec: Codec[EncodedQueryFlags] =
    discriminated[EncodedQueryFlags]
      .by(byte)
      .\(0) { case a @ EncodedQueryFlags(EncodingType.UNCOMPRESSED, _) => a }(
