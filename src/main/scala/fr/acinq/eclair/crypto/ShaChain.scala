...
object ShaChain {
  ...
  def flip(in: ByteVector32, index: Int): ByteVector32 = ByteVector32(
    in.update(index / 8, (in(index / 8) ^ (1 << index % 8)).toByte)
  )

  /** @param index
    *   64-bit integer
    * @return
    *   a binary representation of index as a sequence of 64 booleans. Each bool
    *   represents a move down the tree
    */
  def moves(index: Long): Vector[Boolean] =
    (for (i <- 63 to 0 by -1) yield (index & (1L << i)) != 0).toVector
  ...
  def derive(node: Node, directions: Long): Node =
    derive(node, moves(directions))

  def shaChainFromSeed(hash: ByteVector32, index: Long) =
    derive(Node(hash, 0, None), index).value
  ...
  def getHash(receiver: ShaChain, index: Index): Option[ByteVector32] = {
    receiver.knownHashes.keys
      .find(key => index.startsWith(key))
      .map(key => {
  ...
  def iterator(chain: ShaChain): Iterator[ByteVector32] =
    chain.lastIndex match {
      case None => Iterator.empty
      case Some(index) =>
        new Iterator[ByteVector32] {
          var pos = index
...
/** Structure used to intelligently store unguessable hashes.
  ...
  *   know hashes
  * @param lastIndex
  *   index of the last known hash. Hashes are supposed to be added in reverse
  *   order i.e. from 0xFFFFFFFFFFFFFFFF down to 0
  */
case class ShaChain(
    knownHashes: Map[Vector[Boolean], ByteVector32],
    lastIndex: Option[Long] = None
) {
