...
object Sphinx {
  ...
  def computeEphemeralPublicKeysAndSharedSecrets(
    ...
    val ephemeralPublicKey0 =
      blind(PublicKey(Crypto.curve.getG), sessionKey.value)
  ...
  object RouteBlinding {
    ...
    def create(
      ...
      val (blindedHops, blindingKeys) = publicKeys
        ...
        .map { case (publicKey, payload) =>
          ...
          val rho = generateKey("rho", sharedSecret)
          val (encryptedPayload, mac) =
            ChaCha20Poly1305.encrypt(rho, zeroes(12), payload, ByteVector.empty)
          e = e.multiply(
    ...
    def decryptPayload(
      ...
      val decrypted = ChaCha20Poly1305.decrypt(
        rho,
        zeroes(12),
        encryptedPayload.dropRight(16),
        ByteVector.empty,
        encryptedPayload.takeRight(16)
      )
