...
object Transactions {
  ...
  /** Commitment format as defined in the v1.0 specification
  ...
  /** Commitment format that adds anchor outputs to the commitment transaction
    * and uses custom sighash flags for HTLC transactions to allow unilateral
    * fee bumping (https://github.com/lightningnetwork/lightning-rfc/pull/688).
    */
  ...
  /** When *local* *current* [[CommitTx]] is published:
    *   - [[ClaimLocalDelayedOutputTx]] spends to-local output of [[CommitTx]]
    *     after a delay
    *   - When using anchor outputs, [[ClaimAnchorOutputTx]] spends to-local
    *     anchor of [[CommitTx]]
    *   - [[HtlcSuccessTx]] spends htlc-received outputs of [[CommitTx]] for
    *     which we have the preimage
    *     - [[ClaimLocalDelayedOutputTx]] spends [[HtlcSuccessTx]] after a delay
    *   - [[HtlcTimeoutTx]] spends htlc-sent outputs of [[CommitTx]] after a
    *     timeout
    *     - [[ClaimLocalDelayedOutputTx]] spends [[HtlcTimeoutTx]] after a delay
    *
    * When *remote* *current* [[CommitTx]] is published:
    *   - When using the default commitment format, [[ClaimP2WPKHOutputTx]]
    *     spends to-local output of [[CommitTx]]
    *   - When using anchor outputs, [[ClaimRemoteDelayedOutputTx]] spends
    *     to-local output of [[CommitTx]]
    *   - When using anchor outputs, [[ClaimAnchorOutputTx]] spends to-local
    *     anchor of [[CommitTx]]
    *   - [[ClaimHtlcSuccessTx]] spends htlc-received outputs of [[CommitTx]]
    *     for which we have the preimage
    *   - [[ClaimHtlcTimeoutTx]] spends htlc-sent outputs of [[CommitTx]] after
    *     a timeout
    *
    ...
    *   - When using anchor outputs, [[ClaimAnchorOutputTx]] spends to-local
    *     anchor of [[CommitTx]]
    *   - [[MainPenaltyTx]] spends remote main output using the per-commitment
    *     secret
    *   - [[HtlcSuccessTx]] spends htlc-sent outputs of [[CommitTx]] for which
    *     they have the preimage (published by remote)
    *     - [[ClaimHtlcDelayedOutputPenaltyTx]] spends [[HtlcSuccessTx]] using
    *       the revocation secret (published by local)
    *   - [[HtlcTimeoutTx]] spends htlc-received outputs of [[CommitTx]] after a
    *     timeout (published by remote)
    *     - [[ClaimHtlcDelayedOutputPenaltyTx]] spends [[HtlcTimeoutTx]] using
    *       the revocation secret (published by local)
  ...
  val htlcPenaltyWeight =
    578 // based on spending an HTLC-Success output (would be 571 with HTLC-Timeout)

  def weight2feeMsat(feeratePerKw: FeeratePerKw, weight: Int): MilliSatoshi =
    MilliSatoshi(feeratePerKw.toLong * weight)

  def weight2fee(feeratePerKw: FeeratePerKw, weight: Int): Satoshi =
    weight2feeMsat(feeratePerKw, weight).truncateToSatoshi
  ...
  /** @param fee
    ...
    */
  def fee2rate(fee: Satoshi, weight: Int): FeeratePerKw = FeeratePerKw(
    (fee * 1000L) / weight
  )

  /** Offered HTLCs below this amount will be trimmed. */
  def offeredHtlcTrimThreshold(
      dustLimit: Satoshi,
      spec: CommitmentSpec,
      commitmentFormat: CommitmentFormat
  ): Satoshi =
    dustLimit + weight2fee(
      spec.feeratePerKw,
  ...
  def htlcOutputFee(
    ...
    weight2feeMsat(feeratePerKw, commitmentFormat.htlcOutputWeight)

  /** While fees are generally computed in Satoshis (since this is the smallest
    * on-chain unit), it may be useful in some cases to calculate it in
    * MilliSatoshi to avoid rounding issues. If you are adding multiple fees
    * together for example, you should always add them in MilliSatoshi and then
    * round down to Satoshi.
    */
  def commitTxFeeMsat(
      dustLimit: Satoshi,
      spec: CommitmentSpec,
    ...
    val anchorsCost = commitmentFormat match {
      ...
      case AnchorOutputsCommitmentFormat =>
        AnchorOutputsCommitmentFormat.anchorAmount * 2
    }
    val weight =
      commitmentFormat.commitWeight + commitmentFormat.htlcOutputWeight * (trimmedOfferedHtlcs.size + trimmedReceivedHtlcs.size)
  ...
  def commitTxFee(
    ...
    commitTxFeeMsat(dustLimit, spec, commitmentFormat).truncateToSatoshi

  /** @param commitTxNumber
    *   commit tx number
    * @param isFunder
    *   true if local node is funder
    * @param localPaymentBasePoint
    *   local payment base point
    * @param remotePaymentBasePoint
    *   remote payment base point
    * @return
    *   the obscured tx number as defined in BOLT #3 (a 48 bits integer)
    */
  def obscuredCommitTxNumber(
      commitTxNumber: Long,
      isFunder: Boolean,
    ...
    val blind = Protocol.uint64(
      (h.takeRight(6).reverse ++ ByteVector.fromValidHex("0000")).toArray,
  ...
  /** @param commitTx
    ...
    *   remote payment base point
    * @return
    *   the actual commit tx number that was blinded and stored in locktime and
    *   sequence fields
    */
  def getCommitTxNumber(
    ...
    val blind = obscuredCommitTxNumber(
      0,
      isFunder,
    ...
    val obscured =
      decodeTxNumber(commitTx.txIn.head.sequence, commitTx.lockTime)
    obscured ^ blind
  }

  /** This is a trick to split and encode a 48-bit txnumber into the sequence
    * and locktime fields of a tx
    *
    * @param txnumber
    *   commitment number
    * @return
  ...
  def getHtlcTxInputSequence(commitmentFormat: CommitmentFormat): Long =
    commitmentFormat match {
      case DefaultCommitmentFormat =>
        0 // htlc txs immediately spend the commit tx
      case AnchorOutputsCommitmentFormat =>
        1 // htlc txs have a 1-block delay to allow CPFP carve-out on anchors
    }

  /** Represent a link between a commitment spec item (to-local, to-remote,
    * anchors, htlc) and the actual output in the commit tx
    *
    * @param output
    *   transaction output
    * @param redeemScript
    *   redeem script that matches this output (most of them are p2wsh)
    * @param commitmentOutput
    *   commitment spec item this output is built from
    */
  case class CommitmentOutputLink[T <: CommitmentOutput](
      output: TxOut,
      redeemScript: Seq[ScriptElt],
      commitmentOutput: T
  )
  ...
  object CommitmentOutputLink {
    ...
    /** We sort HTLC outputs according to BIP69 + CLTV as tie-breaker for
      * offered HTLC, we do this only for the outgoing HTLC because we must
      * agree with the remote on the order of HTLC-Timeout transactions even for
      * identical HTLC outputs. See
      * https://github.com/lightningnetwork/lightning-rfc/issues/448#issuecomment-432074187.
      */
    def sort(
        a: CommitmentOutputLink[CommitmentOutput],
        b: CommitmentOutputLink[CommitmentOutput]
    ): Boolean = (a.commitmentOutput, b.commitmentOutput) match {
      case (OutHtlc(OutgoingHtlc(htlcA)), OutHtlc(OutgoingHtlc(htlcB)))
          if htlcA.paymentHash == htlcB.paymentHash && htlcA.amountMsat.truncateToSatoshi == htlcB.amountMsat.truncateToSatoshi =>
        htlcA.cltvExpiry <= htlcB.cltvExpiry
      case _ => LexicographicalOrdering.isLessThan(a.output, b.output)
    }
  }

  def makeCommitTxOutputs(
    ...
    val outputs = collection.mutable.ArrayBuffer
      .empty[CommitmentOutputLink[CommitmentOutput]]

    trimOfferedHtlcs(localDustLimit, spec, commitmentFormat).foreach { htlc =>
      val redeemScript = htlcOffered(
        localHtlcPubkey,
        remoteHtlcPubkey,
        localRevocationPubkey,
        ripemd160(htlc.add.paymentHash.bytes),
        commitmentFormat
      )
      outputs.append(
        CommitmentOutputLink(
          TxOut(htlc.add.amountMsat.truncateToSatoshi, pay2wsh(redeemScript)),
          redeemScript,
          OutHtlc(htlc)
        )
      )
    ...
    val (toLocalAmount: Satoshi, toRemoteAmount: Satoshi) = if (localIsFunder) {
      (
        spec.toLocal.truncateToSatoshi - commitTxFee(
          localDustLimit,
          spec,
          commitmentFormat
        ),
        spec.toRemote.truncateToSatoshi
      )
    } else {
      (
        spec.toLocal.truncateToSatoshi,
        spec.toRemote.truncateToSatoshi - commitTxFee(
          localDustLimit,
          spec,
          commitmentFormat
        )
        ...
        case AnchorOutputsCommitmentFormat =>
          outputs.append(
            CommitmentOutputLink(
              TxOut(
                toRemoteAmount,
                pay2wsh(toRemoteDelayed(remotePaymentPubkey))
              ),
    ...
    if (commitmentFormat == AnchorOutputsCommitmentFormat) {
      if (toLocalAmount >= localDustLimit || hasHtlcs) {
        outputs.append(
          CommitmentOutputLink(
            TxOut(
              AnchorOutputsCommitmentFormat.anchorAmount,
              pay2wsh(anchor(localFundingPubkey))
            ),
            anchor(localFundingPubkey),
            ToLocalAnchor
          )
        )
      }
      if (toRemoteAmount >= localDustLimit || hasHtlcs) {
        outputs.append(
          CommitmentOutputLink(
            TxOut(
              AnchorOutputsCommitmentFormat.anchorAmount,
              pay2wsh(anchor(remoteFundingPubkey))
            ),
            anchor(remoteFundingPubkey),
            ToRemoteAnchor
          )
        )
      }
  ...
  def makeCommitTx(
    ...
    val txnumber = obscuredCommitTxNumber(
      commitTxNumber,
      localIsFunder,
      localPaymentBasePoint,
      remotePaymentBasePoint
  ...
  def makeHtlcTimeoutTx(
      ...
      feeratePerKw: FeeratePerKw,
      commitmentFormat: CommitmentFormat
  ): Either[TxGenerationSkipped, HtlcTimeoutTx] = {
    ...
    if (amount < localDustLimit) {
      ...
      val input = InputInfo(
        OutPoint(commitTx, outputIndex),
        commitTx.txOut(outputIndex),
        write(redeemScript)
      )
      Right(
        HtlcTimeoutTx(
          ...
          Transaction(
            ...
            txIn = TxIn(
              input.outPoint,
              ByteVector.empty,
              getHtlcTxInputSequence(commitmentFormat)
            ) :: Nil,
            txOut = TxOut(
              amount,
              pay2wsh(
            ...
            lockTime = htlc.cltvExpiry.underlying
          )
        )
      )
    }
  ...
  def makeHtlcSuccessTx(
      ...
      feeratePerKw: FeeratePerKw,
      commitmentFormat: CommitmentFormat
  ): Either[TxGenerationSkipped, HtlcSuccessTx] = {
  ...
  def makeHtlcTxs(
    ...
    val htlcTimeoutTxs = outputs.zipWithIndex
      .collect { case (CommitmentOutputLink(o, s, OutHtlc(ou)), outputIndex) =>
    ...
    val htlcSuccessTxs = outputs.zipWithIndex
      .collect { case (CommitmentOutputLink(o, s, InHtlc(in)), outputIndex) =>
  ...
  def makeClaimHtlcSuccessTx(
    ...
    val redeemScript = htlcOffered(
      ...
      ripemd160(htlc.paymentHash.bytes),
      commitmentFormat
    )
  ...
  def makeClaimHtlcTimeoutTx(
    ...
    outputs.zipWithIndex.collectFirst {
      ...
      case Some(outputIndex) =>
        ...
        val weight =
          addSigs(ClaimHtlcTimeoutTx(input, tx), PlaceHolderSig).tx.weight()
        val fee = weight2fee(feeratePerKw, weight)
        val amount = input.txOut.amount - fee
        if (amount < localDustLimit) {
          Left(AmountBelowDustLimit)
  ...
  def makeClaimRemoteDelayedOutputTx(
      commitTx: Transaction,
      localDustLimit: Satoshi,
      localPaymentPubkey: PublicKey,
      localFinalScriptPubKey: ByteVector,
    ...
    findPubKeyScriptIndex(commitTx, pubkeyScript, amount_opt = None) match {
      ...
      case Right(outputIndex) =>
        ...
        val weight =
          addSigs(ClaimRemoteDelayedOutputTx(input, tx), PlaceHolderSig).tx
            .weight()
        val fee = weight2fee(feeratePerKw, weight)
        val amount = input.txOut.amount - fee
        if (amount < localDustLimit) {
  ...
  def makeClaimLocalDelayedOutputTx(
      commitTx: Transaction,
      localDustLimit: Satoshi,
      localRevocationPubkey: PublicKey,
      toLocalDelay: CltvExpiryDelta,
    ...
    val redeemScript = toLocalDelayed(
      localRevocationPubkey,
      toLocalDelay,
      localDelayedPaymentPubkey
    )
    ...
    findPubKeyScriptIndex(commitTx, pubkeyScript, amount_opt = None) match {
      ...
      case Right(outputIndex) =>
        ...
        // compute weight with a dummy 73 bytes signature (the largest you can get)
        val weight =
          addSigs(ClaimLocalDelayedOutputTx(input, tx), PlaceHolderSig).tx
            .weight()
        val fee = weight2fee(feeratePerKw, weight)
        val amount = input.txOut.amount - fee
        if (amount < localDustLimit) {
  ...
  def makeClaimHtlcDelayedOutputPenaltyTx(
      htlcTx: Transaction,
      localDustLimit: Satoshi,
      localRevocationPubkey: PublicKey,
      toLocalDelay: CltvExpiryDelta,
      ...
      localFinalScriptPubKey: ByteVector,
      feeratePerKw: FeeratePerKw
  ): Either[TxGenerationSkipped, ClaimHtlcDelayedOutputPenaltyTx] = {
    val redeemScript = toLocalDelayed(
      localRevocationPubkey,
      toLocalDelay,
      localDelayedPaymentPubkey
    ...
    findPubKeyScriptIndex(htlcTx, pubkeyScript, amount_opt = None) match {
      ...
      case Right(outputIndex) =>
        val input = InputInfo(
          OutPoint(htlcTx, outputIndex),
          htlcTx.txOut(outputIndex),
          write(redeemScript)
        )
        // unsigned transaction
        val tx = Transaction(
        ...
        // compute weight with a dummy 73 bytes signature (the largest you can get)
        val weight =
          addSigs(ClaimHtlcDelayedOutputPenaltyTx(input, tx), PlaceHolderSig).tx
            .weight()
        val fee = weight2fee(feeratePerKw, weight)
        val amount = input.txOut.amount - fee
        if (amount < localDustLimit) {
  ...
  def makeMainPenaltyTx(
    ...
    findPubKeyScriptIndex(commitTx, pubkeyScript, amount_opt = None) match {
      ...
      case Right(outputIndex) =>
        ...
        // compute weight with a dummy 73 bytes signature (the largest you can get)
        val weight =
          addSigs(MainPenaltyTx(input, tx), PlaceHolderSig).tx.weight()
        val fee = weight2fee(feeratePerKw, weight)
        val amount = input.txOut.amount - fee
        if (amount < localDustLimit) {
          Left(AmountBelowDustLimit)
  ...
  def makeHtlcPenaltyTx(
    ...
    val input = InputInfo(
      OutPoint(commitTx, htlcOutputIndex),
      commitTx.txOut(htlcOutputIndex),
      redeemScript
    )
    // unsigned transaction
    val tx = Transaction(
  ...
  def makeClosingTx(
    ...
    val (toLocalAmount: Satoshi, toRemoteAmount: Satoshi) = if (localIsFunder) {
      (
        spec.toLocal.truncateToSatoshi - closingFee,
        spec.toRemote.truncateToSatoshi
      )
    } else {
      (
        spec.toLocal.truncateToSatoshi,
        spec.toRemote.truncateToSatoshi - closingFee
      )
    } // NB: we don't care if values are < 0, they will be trimmed if they are < dust limit anyway
  ...
  /** This default sig takes 72B when encoded in DER (incl. 1B for the trailing
    * sig hash), it is used for fee estimation It is 72 bytes because our
    * signatures are normalized (low-s) and will take up 72 bytes at most in DER
    * format
    */
  ...
  private def sign(
    ...
    val sigDER = Transaction.signInput(
      tx,
      inputIndex = 0,
      redeemScript,
      sighashType,
  ...
  def sign(
    ...
    sign(
      txinfo.tx,
      txinfo.input.redeemScript,
      txinfo.input.txOut.amount,
      key,
      sighashType
    )
  }
  ...
  def sign(
      ...
      txOwner: TxOwner,
      commitmentFormat: CommitmentFormat
  ): ByteVector64 = sign(txinfo, key, txinfo.sighash(txOwner, commitmentFormat))
  ...
  def addSigs(
    ...
    val witness = Scripts.witness2of2(
      localSig,
      remoteSig,
      localFundingPubkey,
      remoteFundingPubkey
  ...
  def addSigs(
      ...
      revocationSig: ByteVector64
  ): MainPenaltyTx = {
    val witness = Scripts.witnessToLocalDelayedWithRevocationSig(
      revocationSig,
      mainPenaltyTx.input.redeemScript
    )
    mainPenaltyTx.copy(tx = mainPenaltyTx.tx.updateWitness(0, witness))
  }

  def addSigs(
      htlcPenaltyTx: HtlcPenaltyTx,
      revocationSig: ByteVector64,
      revocationPubkey: PublicKey
  ): HtlcPenaltyTx = {
    val witness = Scripts.witnessHtlcWithRevocationSig(
      revocationSig,
      revocationPubkey,
      htlcPenaltyTx.input.redeemScript
    )
    htlcPenaltyTx.copy(tx = htlcPenaltyTx.tx.updateWitness(0, witness))
  }

  def addSigs(
      htlcSuccessTx: HtlcSuccessTx,
      localSig: ByteVector64,
      remoteSig: ByteVector64,
      paymentPreimage: ByteVector32,
      commitmentFormat: CommitmentFormat
  ): HtlcSuccessTx = {
    val witness = witnessHtlcSuccess(
      localSig,
      remoteSig,
      paymentPreimage,
      htlcSuccessTx.input.redeemScript,
      commitmentFormat
    )
  ...
  def addSigs(
      htlcTimeoutTx: HtlcTimeoutTx,
      localSig: ByteVector64,
      remoteSig: ByteVector64,
      commitmentFormat: CommitmentFormat
  ): HtlcTimeoutTx = {
    val witness = witnessHtlcTimeout(
      localSig,
      remoteSig,
      htlcTimeoutTx.input.redeemScript,
      commitmentFormat
    )
  ...
  def addSigs(
      claimHtlcSuccessTx: ClaimHtlcSuccessTx,
      localSig: ByteVector64,
      paymentPreimage: ByteVector32
  ): ClaimHtlcSuccessTx = {
    val witness = witnessClaimHtlcSuccessFromCommitTx(
      localSig,
      paymentPreimage,
      claimHtlcSuccessTx.input.redeemScript
    )
    claimHtlcSuccessTx.copy(tx =
      claimHtlcSuccessTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimHtlcTimeoutTx: ClaimHtlcTimeoutTx,
      localSig: ByteVector64
  ): ClaimHtlcTimeoutTx = {
    val witness = witnessClaimHtlcTimeoutFromCommitTx(
      localSig,
      claimHtlcTimeoutTx.input.redeemScript
    )
    claimHtlcTimeoutTx.copy(tx =
      claimHtlcTimeoutTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimP2WPKHOutputTx: ClaimP2WPKHOutputTx,
      localPaymentPubkey: PublicKey,
      localSig: ByteVector64
  ): ClaimP2WPKHOutputTx = {
    val witness = ScriptWitness(Seq(der(localSig), localPaymentPubkey.value))
    claimP2WPKHOutputTx.copy(tx =
      claimP2WPKHOutputTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimRemoteDelayedOutputTx: ClaimRemoteDelayedOutputTx,
      localSig: ByteVector64
  ): ClaimRemoteDelayedOutputTx = {
    val witness = witnessClaimToRemoteDelayedFromCommitTx(
      localSig,
      claimRemoteDelayedOutputTx.input.redeemScript
    )
    claimRemoteDelayedOutputTx.copy(tx =
      claimRemoteDelayedOutputTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimDelayedOutputTx: ClaimLocalDelayedOutputTx,
      localSig: ByteVector64
  ): ClaimLocalDelayedOutputTx = {
    val witness = witnessToLocalDelayedAfterDelay(
      localSig,
      claimDelayedOutputTx.input.redeemScript
    )
    claimDelayedOutputTx.copy(tx =
      claimDelayedOutputTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimAnchorOutputTx: ClaimAnchorOutputTx,
      localSig: ByteVector64
  ): ClaimAnchorOutputTx = {
    val witness =
      witnessAnchor(localSig, claimAnchorOutputTx.input.redeemScript)
    claimAnchorOutputTx.copy(tx =
      claimAnchorOutputTx.tx.updateWitness(0, witness)
    )
  }

  def addSigs(
      claimHtlcDelayedPenalty: ClaimHtlcDelayedOutputPenaltyTx,
      revocationSig: ByteVector64
  ): ClaimHtlcDelayedOutputPenaltyTx = {
    val witness = Scripts.witnessToLocalDelayedWithRevocationSig(
      revocationSig,
      claimHtlcDelayedPenalty.input.redeemScript
    )
    claimHtlcDelayedPenalty.copy(tx =
      claimHtlcDelayedPenalty.tx.updateWitness(0, witness)
    )
  }
  ...
  def checkSpendable(txinfo: TransactionWithInputInfo): Try[Unit] = {
    ...
    Try(
      Transaction.correctlySpends(
        txinfo.tx,
        Map(txinfo.input.outPoint -> txinfo.input.txOut),
  ...
  def checkSig(
    ...
    val data = Transaction.hashForSigning(
      txinfo.tx,
      inputIndex = 0,
      txinfo.input.redeemScript,
      sighash,
