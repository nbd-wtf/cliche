...
package object eclair { me =>
    ...
    Try(Base58Check.decode(address)) match {
      ...
      case Failure(base58error) =>
        Try(Bech32.decodeWitnessAddress(address)) match {
          case Success((_, version, _)) if version > 1 =>
            throw new IllegalArgumentException(
              s"invalid version $version in bech32 address"
            )
          case Success((_, 0, bin)) if bin.length != 20 && bin.length != 32 =>
            throw new IllegalArgumentException(
              "hash length in bech32 address must be either 20 or 32 bytes"
            )
          case Success((_, 1, bin)) if bin.length != 32 =>
            throw new IllegalArgumentException(
          ...
          case Failure(bech32error) =>
            throw new IllegalArgumentException(
              s"$address is invalid, bech32error=$bech32error/base58error=$base58error"
            )
          case _ =>
            throw new IllegalArgumentException(
              s"$address does not match our blockchain, base58error=$base58error"
            )
        }
    }
  }
  ...
  implicit object NumericMilliSatoshi extends Numeric[MilliSatoshi] {
    override def parseString(s: String): Option[MilliSatoshi] =
      Try(s.toInt).toOption.map(MilliSatoshi(_))
    ...
    override def times(x: MilliSatoshi, y: MilliSatoshi): MilliSatoshi =
      MilliSatoshi(x.toLong * y.toLong)
