...
class ElectrumClientPool(blockCount: AtomicLong, chainHash: ByteVector32)(
  ...
  whenUnhandled {
    ...
    case Event(Connect, _) =>
      pickAddress(serverAddresses, addresses.values.toSet) foreach { esa =>
        ...
        val client =
          context actorOf Props(classOf[ElectrumClient], resolved, esa.ssl, ec)
  ...
  private def handleHeader(
      ...
      height: Int,
      tip: BlockHeader,
      data: Option[ConnectedData] = None
  ) = {
    ...
    data match {
      case None =>
        ...
        statusListeners.foreach(
          _ ! ElectrumClient.ElectrumReady(height, tip, remoteAddress)
        )
        context.system.eventStream.publish(
          ElectrumClient.ElectrumReady(height, tip, remoteAddress)
        )
        goto(Connected) using ConnectedData(
          connection,
