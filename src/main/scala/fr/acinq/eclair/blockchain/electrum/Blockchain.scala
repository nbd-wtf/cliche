...
case class Blockchain(
  ...
  /** Build a chain of block indexes
    ...
    *   accumulator
    * @return
    *   the chain that starts at the genesis block and ends at index
    */
  @tailrec
  private def buildChain(
      index: BlockIndex,
      acc: Vector[BlockIndex] = Vector.empty[BlockIndex]
  ): Vector[BlockIndex] = {
    index.parent match {
      case None         => index +: acc
  ...
  /** @param height
    *   block height
    * @return
    *   the encoded difficulty that a block at this height should have
    */
  ...
  def getHeader(height: Int): Option[BlockHeader] = if (
    bestchain.nonEmpty && height >= bestchain.head.height && height - bestchain.head.height < bestchain.size
  )
...
object Blockchain {
  val RETARGETING_PERIOD =
    2016 // on bitcoin, the difficulty re-targeting period is 2016 blocks
  val MAX_REORG =
    72 // we assume that there won't be a reorg of more than 72 blocks
  ...
  case class BlockIndex(
    ...
    lazy val logwork =
      if (chainwork == 0) 0.0
      else Math.log(chainwork.doubleValue) / Math.log(2.0)

    override def toString =
      s"BlockIndex($blockId, $height, ${parent.map(_.blockId)}, $logwork)"
  }
  ...
  def fromGenesisBlock(
    ...
    Blockchain(
      chainhash,
      Vector(),
      Map(blockIndex.hash -> blockIndex),
      Vector(blockIndex)
    )
  }
  ...
  def validateHeadersChunk(
    ...
    require(
      height % RETARGETING_PERIOD == 0,
      s"header chunk height $height not a multiple of 2016"
    )
    require(BlockHeader.checkProofOfWork(headers.head))
    headers.tail.foldLeft(headers.head) { case (previous, current) =>
    ...
    if (cpindex < blockchain.checkpoints.length) {
      ...
      blockchain.chainHash match {
        case Block.LivenetGenesisBlock.hash =>
          require(headers.head.bits == checkpoint.nextBits)
        case _ => ()
      }
    }
    ...
    if (cpindex < blockchain.checkpoints.length - 1) {
      ...
      blockchain.chainHash match {
        case Block.LivenetGenesisBlock.hash =>
          val diff = BlockHeader.calculateNextWorkRequired(
            headers.last,
            headers.head.time
          )
  ...
  def addHeadersChunk(
    ...
    if (headers.length > RETARGETING_PERIOD) {
      val blockchain1 = Blockchain.addHeadersChunk(
        blockchain,
        height,
        headers.take(RETARGETING_PERIOD)
      )
      return Blockchain.addHeadersChunk(
        blockchain1,
        height + RETARGETING_PERIOD,
        headers.drop(RETARGETING_PERIOD)
      )
    }
    ...
    height match {
      case _ if height == blockchain.checkpoints.length * RETARGETING_PERIOD =>
        ...
        val chainwork =
          (blockchain.checkpoints(0) +: blockchain.checkpoints.dropRight(1))
            .map(t =>
              BigInt(RETARGETING_PERIOD) * Blockchain.chainWork(t.nextBits)
            )
            .sum
      ...
      case _ if height == blockchain.height + 1 =>
        ...
        require(
          headers.head.hashPreviousBlock == blockchain.bestchain.last.hash
        )
        val blockIndex = BlockIndex(
          headers.head,
          height,
          None,
          blockchain.bestchain.last.chainwork + Blockchain.chainWork(
            headers.head
          )
        )
        val indexes = headers.tail.foldLeft(Vector(blockIndex)) {
          case (indexes1, header) =>
            indexes1 :+ BlockIndex(
              header,
              indexes1.last.height + 1,
              Some(indexes1.last),
              indexes1.last.chainwork + Blockchain.chainWork(header)
            )
        }
  ...
  def addHeader(
    ...
    require(
      ...
      s"invalid proof of work for $header"
    )
    blockchain.headersMap.get(header.hashPreviousBlock) match {
      case Some(parent) if parent.height == height - 1 =>
        ...
        val blockIndex = BlockIndex(
          ...
          height,
          Some(parent),
          parent.chainwork + Blockchain.chainWork(header)
        )
        val headersMap1 =
          blockchain.headersMap + (blockIndex.hash -> blockIndex)
      ...
      case Some(parent) =>
        throw new IllegalArgumentException(
          s"parent for $header at $height is not valid: $parent "
        )
  ...
  def chainWork(target: BigInt): BigInt =
    BigInt(2).pow(256) / (target + BigInt(1))
  ...
  /** Optimize blockchain
    ...
    * @return
    *   a (blockchain, indexes) tuple where headers that are old enough have
    *   been removed and new checkpoints added, and indexes is the list of
    *   header indexes that have been optimized out and must be persisted
    */
  @tailrec
  def optimize(
      blockchain: Blockchain,
      acc: Vector[BlockIndex] = Vector.empty
  ): (Blockchain, Vector[BlockIndex]) = {
    if (blockchain.bestchain.size >= RETARGETING_PERIOD + MAX_REORG) {
      ...
      val checkpoints1 = blockchain.checkpoints :+ CheckPoint(
        saveme.last.hash,
        bestchain1.head.header.bits
      )
      optimize(
        blockchain.copy(
          headersMap = headersMap1,
          bestchain = bestchain1,
  ...
  /** Computes the difficulty target at a given height.
    ...
    *   blockchain
    * @param height
    *   height for which we want the difficulty target
    * @param headerDb
    *   header database
    * @return
    *   the difficulty target for this height
    */
  def getDifficulty(
      blockchain: Blockchain,
      height: Int,
    ...
    blockchain.chainHash match {
      case Block.LivenetGenesisBlock.hash =>
        (height % RETARGETING_PERIOD) match {
          ...
          case _ =>
            blockchain.getHeader(height - 1) orElse headerDb.getHeader(
              height - 1
            ) map (_.bits)
        }
