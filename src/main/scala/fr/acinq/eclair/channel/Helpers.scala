...
object Helpers {
  def validateParamsFundee(open: OpenChannel, commits: NormalCommits): Unit = {
    ...
    if (reserveToFundingRatio > LNParams.maxReserveToFundingRatio)
      throw ChannelReserveTooHigh(
        open.temporaryChannelId,
        reserveToFundingRatio,
        LNParams.maxReserveToFundingRatio
      )
    if (open.maxAcceptedHtlcs < commits.localParams.maxAcceptedHtlcs)
      throw InvalidMinAcceptedHtlcs(
        open.temporaryChannelId,
        open.maxAcceptedHtlcs,
        commits.localParams.maxAcceptedHtlcs
      )
    if (open.maxAcceptedHtlcs > LNParams.maxAcceptedHtlcs)
      throw InvalidMaxAcceptedHtlcs(
        open.temporaryChannelId,
        open.maxAcceptedHtlcs,
        LNParams.maxAcceptedHtlcs
      )
    if (open.dustLimitSatoshis > open.channelReserveSatoshis)
      throw DustLimitTooLarge(
        open.temporaryChannelId,
        open.dustLimitSatoshis,
        open.channelReserveSatoshis
      )
    if (open.dustLimitSatoshis < LNParams.minChanDustLimit)
      throw DustLimitTooSmall(
        open.temporaryChannelId,
        open.dustLimitSatoshis,
        LNParams.minChanDustLimit
      )
    if (open.pushMsat > open.fundingSatoshis)
      throw InvalidPushAmount(
        open.temporaryChannelId,
        open.pushMsat,
        open.fundingSatoshis.toMilliSatoshi
      )
    if (open.toSelfDelay > LNParams.maxToLocalDelay)
      throw ToSelfDelayTooHigh(
        open.temporaryChannelId,
        open.toSelfDelay,
        LNParams.maxToLocalDelay
      )
    if (LNParams.chainHash != open.chainHash)
      throw InvalidChainHash(
        open.temporaryChannelId,
        local = LNParams.chainHash,
        remote = open.chainHash
      )
    if (open.feeratePerKw < FeeratePerKw.MinimumFeeratePerKw)
    ...
    if (
      ...
      throw InvalidFundingAmount(
        ...
        minFunding,
        LNParams.maxFundingSatoshis
      )
    }

    newFeerate(
      LNParams.feeRates.info,
      commits.localCommit.spec,
      LNParams.shouldForceClosePaymentFeerateDiff
    ).foreach { localFeeratePerKw =>
      throw FeerateTooDifferent(
        open.temporaryChannelId,
        localFeeratePerKw,
        open.feeratePerKw
      )
    ...
    val (toLocalMsat, toRemoteMsat) =
      (open.pushMsat, open.fundingSatoshis.toMilliSatoshi - open.pushMsat)
    val invalidReserve =
      toLocalMsat < open.channelReserveSatoshis && toRemoteMsat < open.channelReserveSatoshis
    if (invalidReserve)
      throw ChannelReserveNotMet(
        open.temporaryChannelId,
        toLocalMsat,
        toRemoteMsat,
        open.channelReserveSatoshis
  ...
  def validateParamsFunder(open: OpenChannel, accept: AcceptChannel): Unit = {
    ...
    if (accept.channelReserveSatoshis < open.dustLimitSatoshis)
      throw ChannelReserveBelowOurDustLimit(
        accept.temporaryChannelId,
        accept.channelReserveSatoshis,
        open.dustLimitSatoshis
      )
    if (open.channelReserveSatoshis < accept.dustLimitSatoshis)
      throw DustLimitAboveOurChannelReserve(
        accept.temporaryChannelId,
    ...
    if (accept.maxAcceptedHtlcs > LNParams.maxAcceptedHtlcs)
      throw InvalidMaxAcceptedHtlcs(
        accept.temporaryChannelId,
        accept.maxAcceptedHtlcs,
        LNParams.maxAcceptedHtlcs
      )
    if (accept.dustLimitSatoshis < LNParams.minChanDustLimit)
      throw DustLimitTooSmall(
        accept.temporaryChannelId,
        accept.dustLimitSatoshis,
        LNParams.minChanDustLimit
      )
    if (accept.maxAcceptedHtlcs < open.maxAcceptedHtlcs)
      throw InvalidMinAcceptedHtlcs(
        accept.temporaryChannelId,
        accept.maxAcceptedHtlcs,
        open.maxAcceptedHtlcs
      )
    if (accept.toSelfDelay > LNParams.maxToLocalDelay)
      throw ToSelfDelayTooHigh(
        accept.temporaryChannelId,
        accept.toSelfDelay,
        LNParams.maxToLocalDelay
  ...
  object Funding {
    ...
    def makeFirstCommitTxs(
      ...
      val toLocalMsat =
        if (localParams.isFunder) fundingAmount.toMilliSatoshi - pushMsat
        else pushMsat
      val toRemoteMsat =
        if (localParams.isFunder) pushMsat
      ...
      val commitmentInput = makeFundingInputInfo(
        fundingTxHash,
      ...
      val (remoteCommitTx, _, _) = NormalCommits.makeRemoteTxs(
        channelFeatures,
        0L,
        localParams,
        remoteParams,
      ...
      val (localCommitTx, _, _) = NormalCommits.makeLocalTxs(
        channelFeatures,
        0L,
        localParams,
        remoteParams,
  ...
  def checkLocalCommit(
      d: HasNormalCommitments,
      nextRemoteRevocationNumber: Long
  ): Boolean =
    if (d.commitments.localCommit.index == nextRemoteRevocationNumber + 1)
      true // We are in sync
    else if (d.commitments.localCommit.index > nextRemoteRevocationNumber + 1)
      true // Remote is behind: we return true because things are fine on our side
    else if (d.commitments.localCommit.index == nextRemoteRevocationNumber)
      true // They sent a new commitSig, we have received it but they didn't receive our revocation
  ...
  object Closing {
    ...
    case class CurrentRemoteClose(
        remoteCommit: RemoteCommit,
        remoteCommitPublished: RemoteCommitPublished
    ) extends RemoteClose
    case class NextRemoteClose(
        remoteCommit: RemoteCommit,
        remoteCommitPublished: RemoteCommitPublished
    ) extends RemoteClose
    ...
    def firstClosingFee(
        commitments: NormalCommits,
        localScriptPubkey: ByteVector,
        remoteScriptPubkey: ByteVector,
        feeratePerKw: FeeratePerKw
    ): Satoshi = {
      val dummyClosingTx = Transactions.makeClosingTx(
        ...
        0L.sat,
        0L.sat,
        commitments.localCommit.spec
      )
      val closingWeight = Transaction.weight(
        Transactions
          .addSigs(
            dummyClosingTx,
            invalidPubKey,
    ...
    def nextClosingFee(
        localClosingFee: Satoshi,
        remoteClosingFee: Satoshi
    ): Satoshi = (localClosingFee + remoteClosingFee) / 4 * 2
    ...
    def makeFirstClosingTx(
      ...
      val closingFee = firstClosingFee(
        commitments,
        localScriptPubkey,
        remoteScriptPubkey,
        conf
    ...
    def makeClosingTx(
      ...
      require(
        isValidFinalScriptPubkey(remoteScriptPubkey),
        "invalid remoteScriptPubkey"
      )
      require(
        isValidFinalScriptPubkey(localScriptPubkey),
        "invalid localScriptPubkey"
      )

      val dustLimitSatoshis = commitments.localParams.dustLimit.max(
        commitments.remoteParams.dustLimit
      )
      val closingTx = Transactions.makeClosingTx(
        commitments.commitInput,
        localScriptPubkey,
        remoteScriptPubkey,
        commitments.localParams.isFunder,
      ...
      val localClosingSig = Transactions.sign(
        closingTx,
        commitments.localParams.keys.fundingKey.privateKey,
        TxOwner.Local,
        commitments.channelFeatures.commitmentFormat
      )
      val closingSigned =
        ClosingSigned(commitments.channelId, closingFee, localClosingSig)
      (closingTx, closingSigned)
    }

    def checkClosingSignature(
        commitments: NormalCommits,
        localScriptPubkey: ByteVector,
        remoteScriptPubkey: ByteVector,
        remote: ClosingSigned
    ): Transaction = {
      val lastCommitFeeSatoshi =
        commitments.commitInput.txOut.amount - commitments.localCommit.publishableTxs.commitTx.tx.txOut
          .map(_.amount)
          .sum
      if (remote.feeSatoshis > lastCommitFeeSatoshi)
        throw ChannelTransitionFail(commitments.channelId, remote)

      val localFundingKey = commitments.localParams.keys.fundingKey.publicKey
      val (closingTx, closingSigned) = makeClosingTx(
        commitments,
        localScriptPubkey,
        remoteScriptPubkey,
        remote.feeSatoshis
      ...
      if (Transactions.checkSpendable(signedTx).isFailure)
        throw ChannelTransitionFail(commitments.channelId, remote)
      signedTx.tx
    }
    ...
    private def generateTx(
        attempt: => SkippedOrTxInfo
    ...
    def claimCurrentLocalCommitTxOutputs(
      ...
      require(
        cs.localCommit.publishableTxs.commitTx.tx.txid == tx.txid,
        "Txid mismatch, provided tx is not the current local commit tx"
      )

      val localPerCommitmentPoint =
        cs.localParams.keys.commitmentPoint(cs.localCommit.index)
      val localRevPubkey = Generators.revocationPubKey(
        cs.remoteParams.revocationBasepoint,
        localPerCommitmentPoint
      )
      val localDelayPubkey = Generators.derivePubKey(
        cs.localParams.keys.delayedPaymentKey.publicKey,
        localPerCommitmentPoint
      )
      val feeratePerKwDelayed =
        conf.feeEstimator.getFeeratePerKw(conf.feeTargets.claimMainBlockTarget)
      ...
      val htlcTxes = cs.localCommit.publishableTxs.htlcTxsAndSigs.flatMap {
        ...
        case HtlcTxAndSigs(info: HtlcTimeoutTx, localSig, remoteSig) =>
          generateTx {
            val info1 = Transactions.addSigs(
              info,
              localSig,
    ...
    def claimRemoteCommitTxOutputs(
        cs: NormalCommits,
        remoteCommit: RemoteCommit,
        tx: Transaction,
        feeEstimator: FeeEstimator
    ): RemoteCommitPublished = {
      val remoteCommitTx = NormalCommits
        .makeRemoteTxs(
          cs.channelFeatures,
          remoteCommit.index,
          cs.localParams,
        ...
        ._1
      require(
        remoteCommitTx.tx.txid == tx.txid,
        "txid mismatch, cannot recompute the current remote commit tx"
      )

      val remoteHtlcPubkey = Generators.derivePubKey(
        cs.remoteParams.htlcBasepoint,
        remoteCommit.remotePerCommitmentPoint
      )
      val localHtlcPubkey = Generators.derivePubKey(
        cs.localParams.keys.htlcKey.publicKey,
        remoteCommit.remotePerCommitmentPoint
      )
      val remoteRevocationPubkey = Generators.revocationPubKey(
        cs.localParams.keys.revocationKey.publicKey,
        remoteCommit.remotePerCommitmentPoint
      )
      val remoteDelayedPaymentPubkey = Generators.derivePubKey(
        cs.remoteParams.delayedPaymentBasepoint,
        remoteCommit.remotePerCommitmentPoint
      )
      val localPaymentPubkey = Generators.derivePubKey(
        cs.localParams.keys.paymentKey.publicKey,
        remoteCommit.remotePerCommitmentPoint
      )

      val outputs: CommitmentOutputs =
        makeCommitTxOutputs(
          !cs.localParams.isFunder,
          cs.remoteParams.dustLimit,
          remoteRevocationPubkey,
          cs.localParams.toSelfDelay,
          remoteDelayedPaymentPubkey,
          localPaymentPubkey,
          remoteHtlcPubkey,
          localHtlcPubkey,
          cs.remoteParams.fundingPubKey,
          cs.localParams.keys.fundingKey.publicKey,
          remoteCommit.spec,
          cs.channelFeatures.commitmentFormat
        )
      ...
      val htlcTxes = remoteCommit.spec.htlcs.toList.flatMap {
        case OutgoingHtlc(add: UpdateAddHtlc)
          ...
          generateTx {
            ...
            val claimToSig = cs.localParams.keys.sign(
              _: ClaimHtlcSuccessTx,
              cs.localParams.keys.htlcKey.privateKey,
              remoteCommit.remotePerCommitmentPoint,
              TxOwner.Local,
              format
            )
            val tx1 = Transactions.makeClaimHtlcSuccessTx(
              remoteCommitTx.tx,
              outputs,
              cs.localParams.dustLimit,
              localHtlcPubkey,
        ...
        case IncomingHtlc(add: UpdateAddHtlc) =>
          generateTx {
            ...
            val claimToSig = cs.localParams.keys.sign(
              _: ClaimHtlcTimeoutTx,
              cs.localParams.keys.htlcKey.privateKey,
              remoteCommit.remotePerCommitmentPoint,
              TxOwner.Local,
              format
            )
            val tx1 = Transactions.makeClaimHtlcTimeoutTx(
              remoteCommitTx.tx,
              outputs,
              cs.localParams.dustLimit,
              localHtlcPubkey,
    ...
    def claimRevokedRemoteCommitTxOutputs(
        cs: NormalCommits,
      ...
      val txnumber = obscuredCommitTxNumber(
        ...
        !cs.localParams.isFunder,
        cs.remoteParams.paymentBasepoint,
      ...
      cs.remotePerCommitmentSecrets
        ...
        .map { remotePerCommitmentSecret =>
          val remoteDelayedPaymentPubkey = Generators.derivePubKey(
            cs.remoteParams.delayedPaymentBasepoint,
            remotePerCommitmentSecret.publicKey
          )
          val remoteRevocationPubkey = Generators.revocationPubKey(
            cs.localParams.keys.revocationKey.publicKey,
            remotePerCommitmentSecret.publicKey
          ...
          val mainPenaltyTx = generateTx {
            val claimToSig = cs.localParams.keys.sign(
              _: MainPenaltyTx,
              cs.localParams.keys.revocationKey.privateKey,
              remotePerCommitmentSecret,
              TxOwner.Local,
              cs.channelFeatures.commitmentFormat
            )
            val tx1 = Transactions.makeMainPenaltyTx(
              commitTx,
              cs.localParams.dustLimit,
              remoteRevocationPubkey,
              finalScriptPubKey,
          ...
          val htlcsRedeemScripts =
            for (redeemScript <- received ++ offered)
              yield (
                Script write pay2wsh(redeemScript),
          ...
          val htlcPenaltyTxs = commitTx.txOut.zipWithIndex.toList.flatMap {
            case (txOut, outputIndex)
              ...
              generateTx {
                ...
                val tx1 = Transactions.makeHtlcPenaltyTx(
                  commitTx,
                  outputIndex,
                  htlcsRedeemScriptsMap(txOut.publicKeyScript),
                  cs.localParams.dustLimit,
                  cs.localParams.defaultFinalScriptPubKey,
                  feeratePerKwPenalty
                )
    ...
    def claimRevokedHtlcTxOutputs(
        cs: NormalCommits,
        revokedCommitPublished: RevokedCommitPublished,
        htlcTx: Transaction,
        feeEstimator: FeeEstimator
    ): (Option[Transaction], RevokedCommitPublished) = {
      val isHtlc =
        !(revokedCommitPublished.claimMainOutputTx ++ revokedCommitPublished.mainPenaltyTx ++ revokedCommitPublished.htlcPenaltyTxs)
          .map(_.txid)
          .toSet
          .contains(htlcTx.txid)
      val isSpenFromRevoke = htlcTx.txIn
        .map(_.outPoint.txid)
        .contains(revokedCommitPublished.commitTx.txid)

      if (isHtlc && isSpenFromRevoke) {
        val obscuredTxNumber = decodeTxNumber(
          revokedCommitPublished.commitTx.txIn.head.sequence,
        ...
        val infoOpt = cs.remotePerCommitmentSecrets
          ...
          .flatMap { remotePerCommitmentSecret =>
            ...
            generateTx {
              val claimToSig = cs.localParams.keys.sign(
                _: ClaimHtlcDelayedOutputPenaltyTx,
                cs.localParams.keys.revocationKey.privateKey,
                remotePerCommitmentSecret,
                TxOwner.Local,
                cs.channelFeatures.commitmentFormat
              )
              val tx1 = Transactions.makeClaimHtlcDelayedOutputPenaltyTx(
                htlcTx,
                cs.localParams.dustLimit,
                remoteRevocationPubkey,
                cs.localParams.toSelfDelay,
    ...
    def findTimedOutHtlc(
        tx: Transaction,
        hash160: ByteVector,
        htlcs: Seq[UpdateAddHtlc],
        extractPaymentHash: PartialFunction[ScriptWitness, ByteVector],
        timeoutTxs: Seq[Transaction] = Nil
    ): Option[UpdateAddHtlc] = {
      val matchingTxs = timeoutTxs
        .filter(_.lockTime == tx.lockTime)
        .filter(
          _.txIn.map(_.witness).collect(extractPaymentHash) contains hash160
        )
    ...
    def timedoutHtlcs(
      ...
      if (tx.txid == localCommit.publishableTxs.commitTx.tx.txid)
        (localCommit.spec.outgoingAdds -- untrimmedHtlcs, true)
      else
        (
          tx.txIn
            .map(_.witness)
    ...
    def timedoutHtlcs(
      ...
      if (tx.txid == remoteCommit.txid)
        (remoteCommit.spec.incomingAdds -- untrimmedHtlcs, true)
      else
        (
          tx.txIn
            .map(_.witness)
    ...
    def updateLocalCommitPublished(
      ...
      val relevantOutpoints = at.tx.txIn.map(_.outPoint).filter { outPoint =>
        ...
        val spendsTheCommitTx =
          localCommitPublished.commitTx.txid == outPoint.txid
        ...
        val is3rdStageDelayedTx = localCommitPublished.claimHtlcDelayedTxs
          .map(_.txid)
          .contains(at.tx.txid)
        isCommitTx || spendsTheCommitTx || is3rdStageDelayedTx
      }
      // then we add the relevant outpoints to the map keeping track of which txid spends which outpoint
      localCommitPublished.copy(irrevocablySpent =
        localCommitPublished.irrevocablySpent ++ relevantOutpoints
          .map(out => out -> at)
          .toMap
      )
    }

    def updateRemoteCommitPublished(
        remoteCommitPublished: RemoteCommitPublished,
        at: TxConfirmedAt
    ): RemoteCommitPublished = {
      // even if our txes only have one input, maybe our counterparty uses a different scheme so we need to iterate
      ...
      val relevantOutpoints = at.tx.txIn.map(_.outPoint).filter { outPoint =>
        ...
        val spendsTheCommitTx =
          remoteCommitPublished.commitTx.txid == outPoint.txid
        isCommitTx || spendsTheCommitTx
      }
      // then we add the relevant outpoints to the map keeping track of which txid spends which outpoint
      remoteCommitPublished.copy(irrevocablySpent =
        remoteCommitPublished.irrevocablySpent ++ relevantOutpoints
          .map(out => out -> at)
          .toMap
      )
    }

    def updateRevokedCommitPublished(
        revokedCommitPublished: RevokedCommitPublished,
        at: TxConfirmedAt
    ): RevokedCommitPublished = {
      ...
      val relevantOutpoints = at.tx.txIn.map(_.outPoint).filter { outPoint =>
        ...
        val spendsHtlcOutput = revokedCommitPublished.claimHtlcDelayedPenaltyTxs
          .flatMap(_.txIn)
          .map(_.outPoint)
          .contains(outPoint)
      ...
      // then we add the relevant outpoints to the map keeping track of which txid spends which outpoint
      revokedCommitPublished.copy(irrevocablySpent =
        revokedCommitPublished.irrevocablySpent ++ relevantOutpoints
          .map(out => out -> at)
          .toMap
      )
    }
  ...
  def chainFeePaid(tx: Transaction, data: DATA_CLOSING): Option[Satoshi] = {
    val isCommitTx =
      tx.txIn.map(_.outPoint).contains(data.commitments.commitInput.outPoint)
    val isFeeDefineable =
      tx.txIn.size == 1 && (data.commitments.localParams.isFunder || !isCommitTx)
    ...
    if (isFeeDefineable) {
      ...
      val chanTxMap = (data.balanceRefunds ++ data.paymentLeftoverRefunds)
        .map(channelTx => channelTx.txid -> channelTx)
        .toMap
      val parent =
        if (isCommitTx) Some(data.commitments.commitInput.txOut)
        else chanTxMap.get(outPoint.txid).map(_ txOut outPoint.index.toInt)
