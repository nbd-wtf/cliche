...
case class ReasonableTrampoline(
    packet: IncomingPaymentPacket.NodeRelayPacket,
    secret: PrivateKey
) extends ReasonableResolution {
  val fullTag: FullPaymentTag = FullPaymentTag(
    packet.add.paymentHash,
    packet.outerPayload.paymentSecret,
    PaymentTagTlv.TRAMPLOINE_ROUTED
  )
  val add: UpdateAddHtlc = packet.add
}

case class ReasonableLocal(
    packet: IncomingPaymentPacket.FinalPacket,
    secret: PrivateKey
) extends ReasonableResolution {
  val fullTag: FullPaymentTag = FullPaymentTag(
    packet.add.paymentHash,
    packet.payload.paymentSecret,
    PaymentTagTlv.FINAL_INCOMING
  )
  val add: UpdateAddHtlc = packet.add
}
...
case class CMD_FAIL_HTLC(
    reason: Either[ByteVector, FailureMessage],
    nodeSecret: PrivateKey,
    theirAdd: UpdateAddHtlc
) extends FinalResolution

case class CMD_ADD_HTLC(
    fullTag: FullPaymentTag,
    firstAmount: MilliSatoshi,
    cltvExpiry: CltvExpiry,
    packetAndSecrets: PacketAndSecrets,
    payload: FinalPayload
) extends Command {
  val incompleteAdd: UpdateAddHtlc = UpdateAddHtlc(
...
sealed trait HasNormalCommitments extends PersistentChannelData {
  override def ourBalance: MilliSatoshi =
    commitments.latestReducedRemoteSpec.toRemote
...
case class LocalCommitPublished(
    ...
    claimMainDelayedOutputTx: Option[Transaction],
    htlcSuccessTxs: List[Transaction],
    htlcTimeoutTxs: List[Transaction],
    claimHtlcDelayedTxs: List[Transaction],
    irrevocablySpent: Map[OutPoint, TxConfirmedAt] = Map.empty
) extends ForceCloseCommitPublished {

  lazy val delayedRefundsLeft: Seq[Transaction] =
    (claimMainDelayedOutputTx.toList ++ claimHtlcDelayedTxs).filterNot(
      isIrrevocablySpent
    )
}

case class RemoteCommitPublished(
    commitTx: Transaction,
    claimMainOutputTx: Option[Transaction],
    claimHtlcSuccessTxs: List[Transaction],
    claimHtlcTimeoutTxs: List[Transaction],
    irrevocablySpent: Map[OutPoint, TxConfirmedAt] = Map.empty
) extends ForceCloseCommitPublished {

  lazy val delayedRefundsLeft: Seq[Transaction] =
    claimHtlcTimeoutTxs.filterNot(isIrrevocablySpent)
}

case class RevokedCommitPublished(
    commitTx: Transaction,
    claimMainOutputTx: Option[Transaction],
    mainPenaltyTx: Option[Transaction],
    htlcPenaltyTxs: List[Transaction],
    claimHtlcDelayedPenaltyTxs: List[Transaction],
    irrevocablySpent: Map[OutPoint, TxConfirmedAt] = Map.empty
) extends ForceCloseCommitPublished {

  lazy val delayedRefundsLeft: Seq[Transaction] =
    claimHtlcDelayedPenaltyTxs.filterNot(isIrrevocablySpent)

  lazy val penaltyTxs: Seq[Transaction] =
...
final case class DATA_WAIT_FOR_FUNDING_INTERNAL(
    initFunder: INPUT_INIT_FUNDER,
...
final case class DATA_WAIT_FOR_FUNDING_CREATED(
    initFundee: INPUT_INIT_FUNDEE,
    remoteParams: RemoteParams,
    lastSent: AcceptChannel
) extends ChannelData

final case class DATA_WAIT_FOR_FUNDING_SIGNED(
    remoteInfo: RemoteNodeInfo,
...
final case class DATA_WAIT_FOR_FUNDING_CONFIRMED(
    commitments: NormalCommits,
    fundingTx: Option[Transaction],
    waitingSince: Long,
    lastSent: Either[FundingCreated, FundingSigned],
    deferred: Option[FundingLocked] = None
) extends ChannelData
...
final case class DATA_WAIT_FOR_FUNDING_LOCKED(
    commitments: NormalCommits,
    shortChannelId: Long,
...
final case class DATA_NORMAL(
    commitments: NormalCommits,
    shortChannelId: Long,
    feeUpdateRequired: Boolean = false,
    extParams: List[ByteVector] = Nil,
    localShutdown: Option[Shutdown] = None,
    remoteShutdown: Option[Shutdown] = None
) extends ChannelData
    with HasNormalCommitments {
...
final case class DATA_NEGOTIATING(
    commitments: NormalCommits,
    localShutdown: Shutdown,
    remoteShutdown: Shutdown,
    closingTxProposed: List[DATA_NEGOTIATING.ClosingProposed] = List(Nil),
    bestUnpublishedClosingTxOpt: Option[Transaction] = None
) extends ChannelData
    with HasNormalCommitments {
...
final case class DATA_CLOSING(
    commitments: NormalCommits,
    waitingSince: Long,
    mutualCloseProposed: List[Transaction] = Nil,
  ...
  lazy val forceCloseCommitPublished: Option[ForceCloseCommitPublished] = {
    ...
    val candidates =
      localCommitPublished ++ remoteCommitPublished ++ nextRemoteCommitPublished ++ futureRemoteCommitPublished ++ revokedCommitPublished
...
final case class DATA_WAIT_FOR_REMOTE_PUBLISH_FUTURE_COMMITMENT(
    commitments: NormalCommits,
...
object ChannelKeys {
  def fromPath(master: ExtendedPrivateKey, path: KeyPath): ChannelKeys = {
    val fundingKey =
      derivePrivateKey(chain = path.path :+ hardened(0L), parent = master)
    val revocationKey =
      derivePrivateKey(chain = path.path :+ hardened(1L), parent = master)
    val paymentKey =
      derivePrivateKey(chain = path.path :+ hardened(2L), parent = master)
    val delayedKey =
      derivePrivateKey(chain = path.path :+ hardened(3L), parent = master)
    val htlcKey =
      derivePrivateKey(chain = path.path :+ hardened(4L), parent = master)
    val shaBase =
      derivePrivateKey(chain = path.path :+ hardened(5L), parent = master)
...
case class ChannelKeys(
    path: KeyPath,
    shaSeed: ByteVector32,
    fundingKey: ExtendedPrivateKey,
    revocationKey: ExtendedPrivateKey,
    paymentKey: ExtendedPrivateKey,
    delayedPaymentKey: ExtendedPrivateKey,
    htlcKey: ExtendedPrivateKey
) {
  def sign(
  ...
  def commitmentPoint(index: Long): PublicKey =
    Generators.perCommitPoint(shaSeed, index)
}

final case class LocalParams(
    keys: ChannelKeys,
    dustLimit: Satoshi,
...
case class FeerateTooSmall(
    channelId: ByteVector32,
    remoteFeeratePerKw: FeeratePerKw
) extends RuntimeException {
  override def toString: String =
    s"FeerateTooSmall, remoteFeeratePerKw=$remoteFeeratePerKw"
}

case class DustLimitTooSmall(
    channelId: ByteVector32,
    dustLimit: Satoshi,
    min: Satoshi
) extends RuntimeException {
  override def toString: String =
    s"DustLimitTooSmall, dustLimit=$dustLimit, min=$min"
}

case class DustLimitTooLarge(
    channelId: ByteVector32,
    dustLimit: Satoshi,
    max: Satoshi
) extends RuntimeException {
  override def toString: String =
    s"DustLimitTooLarge, dustLimit=$dustLimit, max=$max"
}

case class InvalidMaxAcceptedHtlcs(
    channelId: ByteVector32,
    maxAcceptedHtlcs: Int,
    max: Int
) extends RuntimeException {
  override def toString: String =
    s"InvalidMaxAcceptedHtlcs, maxAcceptedHtlcs=$maxAcceptedHtlcs, max=$max"
}

case class InvalidMinAcceptedHtlcs(
    channelId: ByteVector32,
    minAcceptedHtlcs: Int,
    min: Int
) extends RuntimeException {
  override def toString: String =
    s"InvalidMinAcceptedHtlcs, minAcceptedHtlcs=$minAcceptedHtlcs, min=$min"
}

case class InvalidChainHash(
    channelId: ByteVector32,
    local: ByteVector32,
    remote: ByteVector32
) extends RuntimeException {
  override def toString: String =
    s"InvalidChainHash, local=$local, remote=$remote"
}

case class InvalidPushAmount(
    channelId: ByteVector32,
    pushAmount: MilliSatoshi,
    max: MilliSatoshi
) extends RuntimeException {
  override def toString: String =
    s"InvalidPushAmount, pushAmount=$pushAmount, max=$max"
}

case class ToSelfDelayTooHigh(
    channelId: ByteVector32,
    toSelfDelay: CltvExpiryDelta,
    max: CltvExpiryDelta
) extends RuntimeException {
  override def toString: String =
    s"ToSelfDelayTooHigh, toSelfDelay=$toSelfDelay, max=$max"
}
...
case class InvalidFundingAmount(
  ...
  override def toString: String =
    s"InvalidFundingAmount, fundingAmount=$fundingAmount, min=$min, max=$max"
}

case class DustLimitAboveOurChannelReserve(
    channelId: ByteVector32,
    dustLimit: Satoshi,
    channelReserve: Satoshi
) extends RuntimeException {
  override def toString: String =
    s"DustLimitAboveOurChannelReserve, dustLimit=$dustLimit, channelReserve=$channelReserve"
}

case class ChannelReserveBelowOurDustLimit(
    channelId: ByteVector32,
    channelReserve: Satoshi,
    dustLimit: Satoshi
) extends RuntimeException {
  override def toString: String =
    s"ChannelReserveBelowOurDustLimit, channelReserve=$channelReserve, dustLimit=$dustLimit"
}
...
case class ChannelReserveNotMet(
  ...
  override def toString: String =
    s"ChannelReserveNotMet, toLocal=$toLocal, toRemote=$toRemote, reserve=$reserve"
}

case class FeerateTooDifferent(
    channelId: ByteVector32,
    localFeeratePerKw: FeeratePerKw,
    remoteFeeratePerKw: FeeratePerKw
) extends RuntimeException {
  override def toString: String =
    s"FeerateTooDifferent, localFeeratePerKw=$localFeeratePerKw, remoteFeeratePerKw=$remoteFeeratePerKw"
}

case class ChannelReserveTooHigh(
    channelId: ByteVector32,
...
case class ExpiredHtlcInNormalChannel(
    ...
    expiredReceivedRevealed: Boolean
) extends RuntimeException {
  override def toString: String =
    s"ChannelTransitionFail, sentExpiredRouted=$sentExpiredRouted, expiredReceivedRevealed=$expiredReceivedRevealed"
}

case class ChannelTransitionFail(
    channelId: ByteVector32,
    message: LightningMessage
) extends RuntimeException {
  override def toString: String =
    s"ChannelTransitionFail, related message=$message"
}
