...
case class Bolt11Invoice(
  ...
  lazy val routingInfo: List[ExtraHops] = tags.collect { case t: RoutingInfo =>
    t.path
  }
  ...
  lazy val relativeExpiry: FiniteDuration = FiniteDuration(
    relativeExpiry_opt.getOrElse(DEFAULT_EXPIRY_SECONDS.toLong),
    TimeUnit.SECONDS
  )

  lazy val minFinalCltvExpiryDelta: Option[CltvExpiryDelta] =
    tags.collectFirst { case cltvExpiry: Bolt11Invoice.MinFinalCltvExpiry =>
...
object Bolt11Invoice {
  ...
  /** Features supported or required for receiving this payment.
    */
  case class InvoiceFeatures(features: Features[FeatureScope])
      extends TaggedField
