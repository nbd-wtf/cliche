...
case class Features[T <: FeatureScope](
    activated: Map[Feature with T, FeatureSupport],
...
object Features {
  ...
  def apply[T <: FeatureScope](
      features: (Feature with T, FeatureSupport)*
  ): Features[T] = Features[T](features.toMap)
  ...
  def areCompatible[T <: FeatureScope](
      ours: Features[T],
      theirs: Features[T]
  ): Boolean = ours.areSupported(theirs) && theirs.areSupported(ours)

  /** returns true if both have at least optional support */
  def canUseFeature[T <: FeatureScope](
