...
case class TxIn(
    ...
    sequence: Long,
    witness: ScriptWitness = ScriptWitness.empty
...
object ScriptWitness extends BtcSerializer[ScriptWitness] {
  ...
  override def write(
    ...
    writeCollection[ByteVector](
      t.stack,
      (b: ByteVector, o: OutputStream, _: Long) => writeScript(b.toArray, o),
      out,
      protocolVersion
    )
...
object Transaction extends BtcSerializer[Transaction] {
  ...
  override def read(input: InputStream, protocolVersion: Long): Transaction = {
    ...
    val (flags, tx1) =
      if (tx.txIn.isEmpty && serializeTxWitness(protocolVersion)) {
        ...
        if (flags == 0 && txIn.nonEmpty)
          throw new RuntimeException(
            "Extended transaction format unnecessarily used"
          )
    ...
    val tx2 = flags match {
      ...
      case 1 =>
        ...
        for (_ <- tx1.txIn.indices)
          witnesses += ScriptWitness.read(input, protocolVersion)
        tx1.updateWitnesses(witnesses.toSeq).copy(lockTime = uint32(input))
      case _ =>
        throw new RuntimeException(s"Unknown transaction optional data $flags")
    }
  ...
  override def write(
    ...
    if (serializeTxWitness(protocolVersion) && tx.hasWitness) {
      ...
      for (i <- tx.txIn.indices)
        ScriptWitness.write(tx.txIn(i).witness, out, protocolVersion)
  ...
  def hashForSigning(
    ...
    if (isHashSingle(sighashType) && inputIndex >= tx.txOut.length) {
      ...
      Crypto.hash256(
        Transaction.write(
          txCopy,
          Transaction.SERIALIZE_TRANSACTION_NO_WITNESS
        ) ++ writeUInt32(sighashType)
      )
    }
  }
...
case class Transaction(
  ...
  lazy val hash: ByteVector32 =
    Crypto.hash256(Transaction.write(this, SERIALIZE_TRANSACTION_NO_WITNESS))
  ...
  def updateWitness(i: Int, witness: ScriptWitness): Transaction =
    this.copy(txIn = txIn.updated(i, txIn(i).copy(witness = witness)))
  ...
  def baseSize(protocolVersion: Long = PROTOCOL_VERSION): Int =
    Transaction.baseSize(this, protocolVersion)

  def totalSize(protocolVersion: Long = PROTOCOL_VERSION): Int =
    Transaction.totalSize(this, protocolVersion)

  def weight(protocolVersion: Long = PROTOCOL_VERSION): Int =
