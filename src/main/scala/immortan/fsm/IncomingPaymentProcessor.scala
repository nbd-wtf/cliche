...
case class IncomingRevealed(preimage: ByteVector32, fullTag: FullPaymentTag)
    extends IncomingProcessorData
case class IncomingAborted(
    failure: Option[FailureMessage] = None,
    fullTag: FullPaymentTag
) extends IncomingProcessorData

class IncomingPaymentReceiver(val fullTag: FullPaymentTag, cm: ChannelMaster)
          ...
          cm.getPreimageMemo.get(fullTag.paymentHash).toOption match {
            case Some(preimage) =>
              becomeRevealed(
                ...
                adds
              ) // Did not ask but fulfill anyway
            case None =>
              becomeAborted(
                IncomingAborted(None, fullTag),
                adds
              ) // Did not ask and there is no preimage
          }

        case Some(info) =>
          info.description.toSelfPreimage match {
            case _
                if info.isIncoming && PaymentStatus.SUCCEEDED == info.status =>
              becomeRevealed(
                info.preimage,
                info.description.queryText,
                adds
              ) // Already revealed, but not finalized
            case Some(selfPreimage)
                if !info.isIncoming && PaymentStatus.SUCCEEDED == info.status =>
              becomeRevealed(
                selfPreimage,
                info.description.queryText,
            ...
            case _
              ...
              becomeAborted(
                IncomingAborted(None, fullTag),
                adds
              ) // Not enough time to react if stalls

            case _ if info.isIncoming && info.prExt.isEnough(lastAmountIn) =>
              info.description.holdPeriodSec match {
                case None =>
                  becomeRevealed(
                    info.preimage,
                    info.description.queryText,
                ...
                case Some(holdPeriodSeconds) if !isHolding =>
                  hold(holdPeriodSeconds)
                case _ => // Do nothing, await for timeout or user action
              }

            case Some(selfPreimage)
                if !info.isIncoming && info.prExt.isEnough(lastAmountIn) =>
              becomeRevealed(
                selfPreimage,
                info.description.queryText,
                adds
            ...
            case _
                if adds.size >= cm.all.values.count(
                  Channel.isOperational
                ) * LNParams.maxInChannelHtlcs =>
              becomeAborted(
                IncomingAborted(None, fullTag),
                adds
              ) // Ran out of slots
            case None if !info.isIncoming =>
              becomeAborted(
                IncomingAborted(None, fullTag),
                adds
              ) // Not an explicit outgoing self-payment
...
class TrampolinePaymentRelayer(val fullTag: FullPaymentTag, cm: ChannelMaster)
    extends IncomingPaymentProcessor
    with OutgoingPaymentListener { self =>
  ...
  def validateRelay(
    ...
    case pkt
        if pkt.innerPayload.invoiceFeatures.isDefined && pkt.innerPayload.paymentSecret.isEmpty =>
      InvalidOnionPayload(UInt64(8), 0) // We do not serve legacy recepients
    case pkt
        if params.relayFee(
          pkt.innerPayload.amountToForward
        ) > lastAmountIn - pkt.innerPayload.amountToForward =>
      TrampolineFeeInsufficient // Proposed trampoline fee is less than required
    case pkt
    ...
    case pkt
        if params.cltvExpiryDelta > expiryIn(
          adds
        ) - pkt.innerPayload.outgoingCltv =>
      TrampolineExpiryTooSoon // Proposed delta is less than required by our node
    case _
        if !adds
          .map(_.add.channelId)
          .flatMap(cm.all.get)
          .forall(Channel.isOperational) =>
      TemporaryNodeFailure // Some channels got into error state, better stop
    case pkt if CltvExpiry(blockHeight) >= pkt.innerPayload.outgoingCltv =>
      TrampolineExpiryTooSoon // Final recepient's CLTV expiry is below current chain height
    case pkt if pkt.innerPayload.amountToForward < params.minMsat =>
      TemporaryNodeFailure // Peer wants to route less than a minimum we have told them about
  }

  def abortedWithError(
      failures: Failures,
      finalNodeId: PublicKey
  ): TrampolineAborted = {
    val finalNodeFailure = failures.collectFirst {
      case remote: RemoteFailure if remote.packet.originNode == finalNodeId =>
        remote.packet.failureMessage
    }
    val routingNodeFailure = failures.collectFirst {
      case remote: RemoteFailure if remote.packet.originNode != finalNodeId =>
        remote.packet.failureMessage
    }
    TrampolineAborted(
      finalNodeFailure orElse routingNodeFailure getOrElse TemporaryNodeFailure,
      fullTag
    )
  }
  ...
  def doProcess(msg: Any): Unit = (msg, data, state) match {
    ...
    case (
      ...
      firstOpt(ins).foreach { packet =>
        ...
        val reserve =
          packet.outerPayload.totalAmount - packet.innerPayload.amountToForward
        val actualEarnings =
          senderData.filter(_.inFlightParts.nonEmpty).map(reserve - _.usedFee)
        // Second, used fee in sender data may be incorrect after restart, use fallback in that case
        val finalEarnings =
          actualEarnings getOrElse LNParams.trampoline.relayFee(
            packet.innerPayload.amountToForward
          )
        cm.payBag.addRelayedPreimageInfo(
          fullTag,
          preimage,
          packet.innerPayload.amountToForward,
          finalEarnings
      ...
      cm.getPreimageMemo.get(fullTag.paymentHash) match {
        ...
        case _ if outs.isEmpty && collectedEnough(ins) =>
          becomeSendingOrAborted(
            ins
          ) // We have collected enough incoming parts: start sending
  ...
  def becomeSendingOrAborted(adds: ReasonableTrampolines): Unit = {
    ...
    result match {
      ...
      case None =>
        ...
        val extraEdges = RouteCalculation.makeExtraEdges(
          inner.invoiceRoutingInfo.getOrElse(Nil),
          inner.outgoingNodeId
        )
        val totalFeeReserve =
          lastAmountIn - inner.amountToForward - LNParams.trampoline.relayFee(
            inner.amountToForward
          )
        val routerConf = LNParams.routerConf.copy(routeMaxCltv =
          expiryIn(
            adds
          ) - inner.outgoingCltv - LNParams.ourRoutingCltvExpiryDelta
        )
        ...
        val allowedChans = cm.all -- adds
          .map(_.add.channelId)
          .flatMap(cm.all.get)
          .flatMap(Channel.chanAndCommitsOpt)
          .map(_.commits.remoteInfo.nodeId)
          .flatMap(cm.allFromNode)
          .map(_.commits.channelId)
        val send = SendMultiPart(
          fullTag,
          chainExpiry = Left(inner.outgoingCltv),
          splitInfo,
          routerConf,
          inner.outgoingNodeId,
          expectedRouteFees = None,
          inner.paymentMetadata,
          totalFeeReserve,
        ...
        if (inner.invoiceFeatures.isDefined)
          cm.opm process send.copy(
            assistedEdges = extraEdges,
            outerPaymentSecret = inner.paymentSecret.get
          )
        else
          cm.opm process send.copy(
