...
object HtlcInfoTable extends Table {
  val (table, sid, commitNumber, paymentHash160, cltvExpiry) =
    ("htlcinfo", "sid", "cnumber", "hash160", "cltv")
  ...
  def createStatements: Seq[String] = {
    ...
    val addIndex1 =
      s"CREATE INDEX IF NOT EXISTS idx1$table ON $table ($commitNumber)"
...
abstract class ChannelAnnouncementTable(val table: String) extends Table {
  val (features, shortChannelId, nodeId1, nodeId2) =
    ("features", "shortchannelid", "nodeid1", "nodeid2")
...
abstract class ChannelUpdateTable(val table: String, val useHeuristics: Boolean)
  ...
  def createStatements: Seq[String] = {
    ...
    val addIndex =
      s"CREATE $UNIQUE INDEX IF NOT EXISTS idx1$table ON $table ($sid, $position)"
...
abstract class ExcludedChannelTable(val table: String) extends Table {
  ...
  val newSql =
    s"INSERT OR IGNORE INTO $table ($shortChannelId, $until) VALUES (?, ?)"
  ...
  def createStatements: Seq[String] = {
    val createTable =
      s"CREATE TABLE IF NOT EXISTS $table($IDAUTOINC, $shortChannelId INTEGER NOT NULL $UNIQUE, $until INTEGER NOT NULL)"
  ...
  // This one must be a method, not a value because of late binding of abstract val
  def killPresentInChans =
    s"DELETE FROM $table WHERE $shortChannelId IN ($selectFromRelatedUpdateTable)"
...
object ChainWalletTable extends Table {
  val (table, info, xPub, data, lastBalance, label) =
    ("cwallet", "info", "xpub", "data", "lastbalance", "label")

  val newSql =
    s"INSERT OR IGNORE INTO $table ($info, $xPub, $data, $lastBalance, $label) VALUES (?, ?, ?, ?, ?)"
...
object RelayTable extends Table {
  val (table, hash, secret, preimage, seenAt, updatedAt, relayed, earned) = (
    "relay",
    "hash",
    "secret",
    "preimage",
  ...
  val newSql =
    s"INSERT OR IGNORE INTO $table ($hash, $secret, $preimage, $seenAt, $updatedAt, $relayed, $earned) VALUES (?, ?, ?, ?, ?, ?, ?)"
  ...
  def createStatements: Seq[String] = {
    ...
    val addIndex1 =
      s"CREATE $UNIQUE INDEX IF NOT EXISTS idx2$table ON $table ($hash, $secret)"
...
object PaymentTable extends Table {
  ...
  private val inserts =
    s"$pr, $preimage, $status, $seenAt, $updatedAt, $description, $action, $hash, $secret, $receivedMsat, $sentMsat, $feeMsat, $balanceMsat, $fiatRates, $chainFeeMsat, $incoming"
  val newSql =
  ...
  val selectRecentSql: String = {
    ...
    val nonFailedOutgoing =
      s"($updatedAt > 0 AND $status <> $ABORTED AND $incoming = 0)" // Select all outgoing payments which are not failed yet
    val recentPendingIncoming =
  ...
  val searchSql =
    s"SELECT * FROM $table WHERE $hash IN (SELECT DISTINCT $hash FROM $fts$table WHERE $search MATCH ? LIMIT 100)"

  // Select both incoming and outgoing payments because outgoing ones may be reflexive
  val selectPendingSql =
    s"SELECT * FROM $table WHERE ($updatedAt > ? AND $status = $PENDING)"
  ...
  val updOkIncomingSql =
    s"UPDATE $table SET $status = $SUCCEEDED, $receivedMsat = ?, $seenAt = ?, $updatedAt = ? WHERE $hash = ? AND ($updatedAt > 0 AND $status <> $SUCCEEDED AND $incoming = 1)"

  val updStatusSql =
    s"UPDATE $table SET $status = ?, $updatedAt = ? WHERE $hash = ? AND ($updatedAt > 0 AND $status <> $SUCCEEDED)"

  val updateDescriptionSql =
    s"UPDATE $table SET $description = ? WHERE $hash = ?"
  ...
  def createStatements: Seq[String] = {
    ...
    val addIndex1 =
      s"CREATE INDEX IF NOT EXISTS idx1$table ON $table ($updatedAt, $status, $incoming)"
...
object TxTable extends Table {
  ...
  private val inserts =
    s"$rawTx, $txid, $pub, $depth, $receivedSat, $sentSat, $feeSat, $seenAt, $updatedAt, $description, $balanceMsat, $fiatRates, $incoming, $doubleSpent"
  val newSql =
    s"INSERT OR IGNORE INTO $table ($inserts) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ...
  def createStatements: Seq[String] = {
    ...
    val addSearchTable =
      s"CREATE VIRTUAL TABLE IF NOT EXISTS $fts$table USING $fts($search, $txid)"
...
object ChannelTxFeesTable extends Table {
  ...
  val (table, identifier, tag, feeSat) =
    ("chantxfees", "identifier", "tag", "fee")

  val newSql =
    s"INSERT OR IGNORE INTO $table ($identifier, $tag, $feeSat) VALUES (?, ?, ?)"
...
object ElectrumHeadersTable extends Table {
  val (table, height, blockHash, header) =
    ("headers", "height", "blockhash", "header")
...
object LNUrlPayTable extends Table {
  ...
  def createStatements: Seq[String] = {
    ...
    val addSearchTable =
      s"CREATE VIRTUAL TABLE IF NOT EXISTS $fts$table USING $fts($search, $domain)"
